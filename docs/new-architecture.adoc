...
リコンサイラ:: （カスタム）リソースを受け取り、そのリソースが理想とする状態になるように処理を行う（リコンサイルをする）関数を指す。controller-runtime が生成する `Reconcile` 関数を含むが、定義に合致すればそれに限定しない。例えばドメイン層の `Provision`・`Finalize` 関数やユースケース層の `Run` 関数も含む。
...
ドメイン層からインターフェースを経由して外部サービスにアクセスする場合、そのインターフェースはその処理に必要なメソッドのみを定義し、他のドメイン層の処理とは共有しない（SHOULD）<<rob-pike-interface>>。
...
リコンサイル対象のリソースを変更したい場合、`Provision`・`Finalize` 関数の引数として渡されたカスタムリソースを変更する。その変更の永続化は `Provision`・`Finalize` 関数の呼び出し側で行う。一方で、リコンサイル対象以外のリソースを変更したり、Kubernetes 以外の外部サービスにアクセスしたい場合、以下のどちらかによって行うfootnote:[
このやり方は DDD の「ユーザーインターフェイスからのリクエストは、ひとつの集約のインスタンスに対する単一のコマンドにして実行しなければならない」（<<implementing-ddd>> P.341）というルールを参考にしている。
]：

* ドメインイベントを作成するfootnote:[
使用するイベントのほとんどは外部サービスに対する「コマンド発行済みイベント」（<<learning-ddd>> P.183）になるだろう。例えば「CronJob を作成する（コマンドを発行した）」とか「MantleBackup を更新する（コマンドを発行した）」など。
]footnote:[
似たアプローチをとっているものとして `OutputSet` がある<<achilles-sdk-output-set>>が、同じモチベーションなのかどうかはよく分からなかった。
]。作成したドメインイベントは関数の実行中は一箇所（構造体のフィールドなど）に溜めておき、関数が完了した後に関数の呼び出し側がその内容を実行する<<a-better-domain-events-pattern>>。
...
以下の理由から、前者のドメインイベントを使用するアプローチが推奨される：

* リコンサイラが引数で受け取った入力のみを用いて処理を行うこと（純粋関数になること）を強制するため、関数の見通しがよくなりやすい。
* リソースの取得と更新を交互に行うようなコードが書けないため、古いキャッシュを掴んだり楽観的ロックに失敗したりする機会が減る。
* 単体テストで fake や mock を使用する必要がない。

一方で、外部サービス呼び出しの結果を元にリコンサイルの内容を変更したい場合は、ドメインイベントでは達成できないため、`interface` を用いるアプローチを採用する。

=== ユースケース層

リコンサイルを必要とする各カスタムリソースに対して、ユースケース層で `Run` 関数が定義・公開されなければならない（MUST）。この関数では以下の処理をこの順番で行う（SHOULDfootnote:[リソースによっては処理の順番が前後したり、追加の処理が必要になるかもしれない。]）：
...
* RDB を使用できないため outbox パターンを用いたドメインイベントの永続化を行えない。そのため、コントローラーが再起動した際などに発行済みのドメインイベントが失われる可能性がある。実装上は、これは `Reconcile` が冪等で、かつ繰り返し呼ばれることによってカバーされる（べき）。
...
* ユニットテストを書きやすくしたい
** e2e テストや envtest が長時間かかる上に flaky なので、できるだけ（高速な）ユニットテストで正しいことを保証したい。これにより開発のイテレーションが回しやすくなるはず。
** Mantle の e2e テストではシナリオテスト（もどき）になっているため、何をテストしたいのかがわかりにくくなっている。回すのに時間がかかるのでおいそれとテストケースを増やしにくい。Arrange-Act-Assert<<xp123-3a>> がはっきりしたユニットテストに変えることで、テストの見通しが良くなり、不足しているテストが何であるかに気づきやすくなるだろう（cf. テストが存在しないことに気づけなかったためにデグレした例 https://github.com/cybozu-go/mantle/pull/233 ）。
* 人にとってわかりやすいリコンサイラを書きたい
** 現在は巨大な Reconciler のための構造体が存在し、これに属するメソッドで全ての処理を行なっている。関心を分離して、Kubernetes クラスタや Ceph クラスタといった外部リソースにアクセスする部分と、コントローラが次に何を行うべきかを判定する部分を分けることで、コードの見通しをよくする。
** 現在の Mantle ではリソースの取得と更新が交互に行われている。その結果、古いキャッシュを掴んだり楽観的ロックに失敗したりして、リコンサイラの挙動が分かりにくくなっている。一度の reconciliation でリソースの取得と更新を一度のみ行うことで、このような問題を（ほぼ）解決できるはず。
** primary・secondary で動くコードを分ける
...
* [[[xp123-3a,10]]] https://xp123.com/3a-arrange-act-assert/
* [[[implementing-ddd,11]]] 『実践ドメイン駆動設計』（ヴォーン・ヴァーノン (著), 髙木 正弘 (翻訳)、翔泳社）
* [[[rob-pike-interface,12]]] https://groups.google.com/g/golang-nuts/c/mg-8_jMyasY/m/lo-kDuEd540J
