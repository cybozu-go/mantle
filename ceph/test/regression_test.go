...
func testRegression() {
	...
	Describe("test export-diff for snapshot without from-snap option", test.test1)
	Describe("test export-diff for snapshot with from-snap option", test.test2)
	Describe("test export-diff for RBD image", test.test3)
...
func (t *regressionTest) test1() {
	It("export-diff for snapshot without from-snap option", func() {
		tests := []struct {
			...
			{
				description:      "(182) specify snapshot name with <image>@<snap> format",
...
func (t *regressionTest) test2() {
	It("export-diff for snapshot with from-snap option", func() {
		...
		err := cluster.ExportDiff("/tmp/exported.bin",
			"-p", t.poolName, fmt.Sprintf("%s@%s", t.srcImageName, t.snapshots[0]))
		Expect(err).NotTo(HaveOccurred())
		err = cluster.ImportDiff("/tmp/exported.bin", t.poolName, t.dstImageName,
			"", t.namespace, t.dstDeployName, t.dstPVCName)
		Expect(err).NotTo(HaveOccurred())
		...
		tests := []struct {
			...
			{
				description:      "(186) specify snapshot name with <image>@<snap> format",
		...
		for i := 1; i < 4; i++ {
			err := cluster.ExportDiff("/tmp/exported.bin",
				"-p", t.poolName,
				"--from-snap", t.snapshots[i-1],
				fmt.Sprintf("%s@%s", t.srcImageName, t.snapshots[i]))
			Expect(err).NotTo(HaveOccurred())
			err = cluster.ImportDiff("/tmp/exported.bin", t.poolName, t.dstImageName,
				t.snapshots[i-1], t.namespace, t.dstDeployName, "")
			Expect(err).NotTo(HaveOccurred())
		}
		// apply snapshot to the destination image
