...
snapshot_exists() {
    ...
    # rbd prints at least an empty array, so an empty output is an error here.
    # Note that the exit status of jq -e cannot tell such an empty input from
    # an invalid one, because jq 1.6 exits with 0 for the former and with 4 for
    # the latter, while jq 1.7 and later do the opposite. So we look at the
    # printed value instead, and treat everything else as a failure.
    result=$(printf '%s' "${snaps}" |
        jq --arg name "$1" 'any(.[]; .name == $name)') || result=""
    case "${result}" in
        true) return 0 ;;
        false) return 1 ;;
    esac
