...
func reconcileSucceeded() *reconcileResult {
...
func reconcileFailed(format string, args ...any) *reconcileResult {
	return &reconcileResult{err: fmt.Errorf(format, args...)}
}

func (r *reconcileResult) shouldReturn() bool {
	if r == nil {
		return false
	}
...
func (r *reconcileResult) wrapIfError(format string, args ...any) *reconcileResult {
	...
	return reconcileFailed(format+": %w", append(args, r.err)...)
}

func (r *reconcileResult) toCtrlResult() (ctrl.Result, error) {
	// success
	if r == nil || r.success {
