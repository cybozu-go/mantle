...
case "$1 $2" in
    ...
    "snap ls")
        ...
        if [ "${STUB_SNAP_LS_EMPTY}" = "true" ]; then
            exit 0
        fi
        warn_if_requested
        print_snaps_json
...
func TestImportScriptStopsOnRBDError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// rbd prints at least an empty array, so an empty output means
			// that something went wrong. It must not be taken as the
			// existence of the snapshot, which would skip the import.
			name: "the snapshot list is empty",
			env:  map[string]string{"FROM_SNAP_NAME": snapName1, "STUB_SNAP_LS_EMPTY": "true"},
		},
		{
