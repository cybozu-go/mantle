...
func (r *MantleBackupReconciler) checkPVCInManagedCluster(ctx context.Context, pvc *corev1.PersistentVolumeClaim) *reconcile.Result {
	logger := log.FromContext(ctx)
	...
	if err != nil {
		...
		return reconcile.Failed("failed to get clusterID from PVC: %w", err)
	}
...
func (r *MantleBackupReconciler) reconcileLocalBackup(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	...
	if _, ok := backup.Labels[labelLocalBackupTargetPVCUID]; !ok {
		...
		if err := r.Update(ctx, backup); err != nil {
			...
			return reconcile.Failed("failed to add label: %w", err)
		}
	}
	...
	if !controllerutil.ContainsFinalizer(backup, MantleBackupFinalizerName) {
		...
		if err := r.Update(ctx, backup); err != nil {
			...
			return reconcile.Failed("failed to add finalizer: %w", err)
		}
