...
const (
	...
	// labelClusterID identifies the controller that should reconcile the
	// MantleBackup, guaranteeing there is never a MantleBackup that has a
	// non-nil deletionTimestamp but no controller responsible for it. To
	// uphold this guarantee, the label is attached at the same time as the
	// finalizer, and any cleanup that must run (e.g. deleting the RBD snapshot)
	// is only performed after both this label and the finalizer are attached.
...
func (r *MantleBackupReconciler) removeRBDSnapshot(ctx context.Context, poolName, imageName, snapshotName string) *reconcile.Result {
	logger := log.FromContext(ctx)
	var imageID string
	imageNames, err := r.ceph.RBDLs(poolName)
	if err != nil {
		return reconcile.Failed("failed to list RBD images: %w", err)
	}
	if slices.Contains(imageNames, imageName) {
		info, err := r.ceph.RBDInfo(poolName, imageName)
		if err != nil {
			return reconcile.Failed("failed to get RBD info: %w", err)
		}
		imageID = info.ID
	} else {
		trashList, err := r.ceph.RBDTrashLs(poolName)
		if err != nil {
			return reconcile.Failed("failed to list RBD trash: %w", err)
		}
	...
	if rmErr != nil {
		...
		if lsErr != nil {
			return reconcile.Failed("failed to ensure rbd snapshot is removed: %s/%s@%s: %w", poolName, imageID, snapshotName, errors.Join(rmErr, lsErr))
		}
		for _, snap := range snapshots {
			if snap.Name == snapshotName {
				return reconcile.Failed("failed to remove rbd snapshot: %w", rmErr)
			}
		}
...
func (r *MantleBackupReconciler) createRBDSnapshot(
...
) (*ceph.RBDSnapshot, *reconcile.Result) {
	logger := log.FromContext(ctx)
	...
	if findErr != nil {
		...
		return nil, reconcile.Failed("failed to find rbd snapshot: %w", errors.Join(createErr, findErr))
	}

	return snap, nil
}

func (r *MantleBackupReconciler) checkPVCBound(ctx context.Context, pvc *corev1.PersistentVolumeClaim) *reconcile.Result {
	logger := log.FromContext(ctx)
	if pvc.Status.Phase != corev1.ClaimBound {
		if pvc.Status.Phase == corev1.ClaimPending {
			...
			return reconcile.Failed("PVC phase is neither bound nor pending (status.phase: %s)", pvc.Status.Phase)
		}
	}
...
func (r *MantleBackupReconciler) getSnapshotTarget(ctx context.Context, backup *mantlev1.MantleBackup) (
	...
	*reconcile.Result,
	...
	if uid, ok := backup.GetLabels()[labelLocalBackupTargetPVCUID]; ok && uid != string(pvc.GetUID()) {
		return nil, reconcile.Failed("PVC UID does not match the backup target")
	}
	...
	if err := r.Get(ctx, types.NamespacedName{Name: pvName}, &pv); err != nil {
		logger.Error(err, "failed to get PV", "pv", pvName, "pvc", pvcName)
	...
	if !ok {
		return nil, reconcile.Failed("failed to get imageName from PV")
	}
	poolName, ok := pv.Spec.CSI.VolumeAttributes["pool"]
	if !ok {
		return nil, reconcile.Failed("failed to get pool from PV")
	}
...
func (r *MantleBackupReconciler) expire(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	logger := log.FromContext(ctx)
	...
	if err != nil {
		return reconcile.Failed("failed to parse expire duration: %w", err)
	}
	...
	if time.Now().UTC().After(expireAt) {
		...
		if err := r.Delete(ctx, backup); err != nil && !aerrors.IsNotFound(err) {
			return reconcile.Failed("failed to delete expired backup: %w", err)
		}
...
func (r *MantleBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	logger = logger.WithValues("backupUID", string(backup.GetUID()))
...
func (r *MantleBackupReconciler) checkManagedBackup(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	...
	if err := r.Get(ctx, types.NamespacedName{Namespace: backup.Namespace, Name: backup.Spec.PVC}, &pvc); err != nil {
		...
		return reconcile.Failed("failed to get the PVC specified in the MantleBackup resource: %w", err)
	}

	clusterID, err := getCephClusterIDFromPVC(ctx, r.Client, &pvc)
	if err != nil {
		return reconcile.Failed("failed to get the Ceph cluster ID from the PVC: %w", err)
	}
...
func (r *MantleBackupReconciler) reconcileLocalBackup(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	...
	if updated {
		if err := r.Update(ctx, backup); err != nil {
			...
			return reconcile.Failed("failed to update MantleBackup: %w", err)
		}
	}
...
func (r *MantleBackupReconciler) reconcileAsStandalone(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
...
func (r *MantleBackupReconciler) reconcileAsPrimary(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	if result := r.reconcileLocalBackup(ctx, backup); result.ShouldReturn() {
		return result.WrapIfError("failed to reconcile local backup")
	}
...
func (r *MantleBackupReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		WithOptions(controller.TypedOptions[ctrlreconcile.Request]{
			RateLimiter: newMantleBackupReconcilerRateLimiter[ctrlreconcile.Request](),
		}).
...
func (r *MantleBackupReconciler) replicateManifests(
	...
	if err := json.Unmarshal([]byte(backup.Status.PVCManifest), &pvc); err != nil {
		return reconcile.Failed("failed to unmarshal the PVC stored in the status of the MantleBackup resource: %w", err)
	}

	// Make sure the arguments are valid
	if backup.Status.SnapID == nil {
		return reconcile.Failed("backup.Status.SnapID should not be nil: %s: %s", backup.GetName(), backup.GetNamespace())
	}
	...
	if err != nil {
		return reconcile.Failed("failed to parse quantity: %w", err)
	}
	...
	if err != nil {
		return reconcile.Failed("failed to marshal the PVC to be sent to the secondary mantle: %w", err)
	}
	...
	if err != nil {
		return reconcile.Failed("failed to call CreateOrUpdatePVC: %w", err)
	}
	...
	if err != nil {
		return reconcile.Failed("failed to marshal the MantleBackup to be sent to the secondary mantle: %w", err)
	}
	...
	if _, err := client.CreateMantleBackup(
		...
		return reconcile.Failed("failed to call CreateMantleBackup: %w", err)
	}
...
func (r *MantleBackupReconciler) verify(
	...
	if err := json.Unmarshal([]byte(backup.Status.PVCManifest), &storedPVC); err != nil {
		return reconcile.Failed("failed to unmarshal PVC manifest: %w", err)
	}

	var storedPV corev1.PersistentVolume
	if err := json.Unmarshal([]byte(backup.Status.PVManifest), &storedPV); err != nil {
		return reconcile.Failed("failed to unmarshal PV manifest: %w", err)
	}
	...
	if err := createCloneByPV(ctx, r.ceph, &storedPV, backup.Name, MakeVerifyImageName(backup)); err != nil {
		return reconcile.Failed("failed to create a clone by the snapshot: %w", err)
	}
...
// It returns (is job finished, is job succeeded, reconcile.Result).
func (r *MantleBackupReconciler) checkJobStatus(ctx context.Context, jobName string) (bool, bool, *reconcile.Result) {
	var job batchv1.Job
	if err := r.Get(
		ctx,
		...
		return false, false, reconcile.Failed("failed to get Job for checking job status: %w", err)
	}
...
func (r *MantleBackupReconciler) prepareForDataSynchronization(
...
) (*dataSyncPrepareResult, *reconcile.Result) {
	exportTargetPVCUID, ok := backup.GetLabels()[labelLocalBackupTargetPVCUID]
	if !ok {
		return nil, reconcile.Failed(`"%s" label is missing`, labelLocalBackupTargetPVCUID)
	}
	...
	if err != nil {
		return nil, reconcile.Failed("failed to list MantleBackup in the secondary mantle: %w", err)
	}

	secondaryBackups := make([]mantlev1.MantleBackup, 0)
	if err = json.Unmarshal(resp.GetMantleBackupList(), &secondaryBackups); err != nil {
		return nil, reconcile.Failed("failed to unmarshal the MantleBackup list: %w", err)
	}
	secondaryBackupMap := convertToMap(secondaryBackups)

	secondaryBackup, ok := secondaryBackupMap[backup.GetName()]
	if !ok {
		return nil, reconcile.Failed("secondary MantleBackup not found: %s, %s",
	...
	if syncMode, ok := backup.GetAnnotations()[annotSyncMode]; ok {
		...
		case syncModeIncremental:
			...
			if !ok {
				return nil, reconcile.Failed(`"%s" annotation is missing`, annotDiffFrom)
			}
			...
			if err := r.Get(ctx, types.NamespacedName{
				...
				return nil, reconcile.Failed("failed to get diff-from MantleBackup: %w", err)
			}
		...
		default:
			return nil, reconcile.Failed("unknown sync mode: %s", syncMode)
		}
	}
	...
	if err := r.List(ctx, &primaryBackupList, &client.ListOptions{
		...
		return nil, reconcile.Failed("failed to list primary MantleBackup: %w", err)
	}
...
func (r *MantleBackupReconciler) startExportAndUpload(
	...
	if _, err := r.primarySettings.Client.SetSynchronizing(
		...
		return reconcile.Failed("failed to call SetSynchronizing RPC: %w", err)
	}
...
func (r *MantleBackupReconciler) annotateExportTargetMantleBackup(
	...
	if err := r.Get(ctx, key, target); err != nil {
		return reconcile.Failed("failed to get export target MantleBackup: %w", err)
	}
	...
	if err := r.Update(ctx, target); err != nil {
		return reconcile.Failed("failed to update export target MantleBackup: %w", err)
	}
...
func (r *MantleBackupReconciler) annotateExportSourceMantleBackup(
	...
	if err := r.Get(ctx, key, source); err != nil {
		return reconcile.Failed("failed to get export source MantleBackup: %w", err)
	}
	...
	if err := r.Update(ctx, source); err != nil {
		return reconcile.Failed("failed to update export source MantleBackup: %w", err)
	}
...
func (r *MantleBackupReconciler) canNewJobBeCreated(ctx context.Context, maxJobs int, component string) (bool, *reconcile.Result) {
	if maxJobs == 0 {
		return true, nil
	}
...
func (r *MantleBackupReconciler) getPoolAndImageFromStatusPVManifest(backup *mantlev1.MantleBackup) (string, string, *reconcile.Result) {
	var pv corev1.PersistentVolume
	if err := json.Unmarshal([]byte(backup.Status.PVManifest), &pv); err != nil {
		return "", "", reconcile.Failed("failed to unmarshal status.PVManifest: %w", err)
	}

	return pv.Spec.CSI.VolumeAttributes["pool"], pv.Spec.CSI.VolumeAttributes["imageName"], nil
}

func (r *MantleBackupReconciler) getNumberOfParts(backup *mantlev1.MantleBackup) (int, *reconcile.Result) {
	if backup.Status.SnapSize == nil {
		return 0, reconcile.Failed("failed to get status.snapSize: %s/%s", backup.GetNamespace(), backup.GetName())
	}
	if backup.Status.TransferPartSize == nil {
		return 0, reconcile.Failed("failed to get status.transferPartSize: %s/%s", backup.GetNamespace(), backup.GetName())
	}

	transferPartSize, ok := backup.Status.TransferPartSize.AsInt64()
	if !ok {
		return 0, reconcile.Failed("failed to convert transferPartSize to int64: %s/%s: %s",
...
func (r *MantleBackupReconciler) haveAllExportJobsCompleted(backup *mantlev1.MantleBackup, largestCompletedPartNum int) (bool, *reconcile.Result) {
...
func (r *MantleBackupReconciler) startUpload(ctx context.Context, targetBackup *mantlev1.MantleBackup, largestCompletedExportPartNum int) *reconcile.Result {
...
func (r *MantleBackupReconciler) createOrUpdateExportDataPVC(
	...
	if _, err = ctrl.CreateOrUpdate(ctx, r.Client, &pvc, func() error {
		...
		return reconcile.Failed("failed to create or update export data PVC: %w", err)
	}
...
func (r *MantleBackupReconciler) createOrUpdateExportJob(
	...
	if !ok {
		return reconcile.Failed("failed to convert transferPartSize to int64: %d", transferPartSize)
	}
	...
	if _, err := ctrl.CreateOrUpdate(ctx, r.Client, &job, func() error {
		...
		return reconcile.Failed("failed to create or update export Job: %w", err)
	}
...
func (r *MantleBackupReconciler) createOrUpdateUploadJobs(
	...
	for partNum := minPartNum; partNum <= maxPartNum; partNum++ {
		...
		if _, err := ctrl.CreateOrUpdate(ctx, r.Client, &job, func() error {
			...
			return reconcile.Failed("failed to create or update upload Job: %w", err)
		}
	}
...
func (r *MantleBackupReconciler) startImport(
	...
	logger := log.FromContext(ctx).WithValues("pool", target.poolName, "image", target.imageName)
	ctx = log.IntoContext(ctx, logger)
...
func (r *MantleBackupReconciler) prepareObjectStorageClient(ctx context.Context) *reconcile.Result {
	...
	if r.objectStorageSettings.CACertConfigMap != nil {
		...
		if err := r.Get(
			...
			return reconcile.Failed("failed to get ConfigMap for ca-cert-key: %w", err)
		}

		caPEMCertsString, ok := cm.Data[*r.objectStorageSettings.CACertKey]
		if !ok {
			return reconcile.Failed("ca-cert-key not found in ConfigMap: %s", *r.objectStorageSettings.CACertConfigMap)
		}
...
func (r *MantleBackupReconciler) handleCompletedImportJobs(ctx context.Context, backup *mantlev1.MantleBackup) (int, *reconcile.Result) {
...
func (r *MantleBackupReconciler) lockVolume(
	...
	if errAdd := r.ceph.RBDLockAdd(poolName, imageName, lockID); errAdd != nil {
		...
		if errLs != nil {
			return false, reconcile.Failed("failed to add a lock and list locks on volume %s/%s: %w", poolName, imageName, errors.Join(errAdd, errLs))
		}

		switch len(locks) {
		case 0:
			// It may have been unlocked after the lock failed, but since other causes are also possible, an error is returned.
			return false, reconcile.Failed("failed to add a lock to the volume %s/%s: %w", poolName, imageName, errAdd)
		...
		default:
			...
			return false, reconcile.Failed("multiple locks found on volume %s/%s after failed lock attempt(%v)", poolName, imageName, locks)
		}
	}
...
func (r *MantleBackupReconciler) unlockVolume(
	...
	if err != nil {
		return reconcile.Failed("failed to list locks of the volume %s/%s: %w", poolName, imageName, err)
	}

	if len(locks) >= 2 {
		return reconcile.Failed("multiple locks found on volume %s/%s when unlocking (%v)", poolName, imageName, locks)
	}
...
func (r *MantleBackupReconciler) createStaticPVIfNotExists(
	...
	if basePV.Spec.CSI == nil {
		return reconcile.Failed("PV is not a CSI volume")
	}
...
`,
		...
		return reconcile.Failed("failed to create or update zeroout Job: %w", err)
	}

	return nil
}

func (r *MantleBackupReconciler) createOrUpdateVerifyJob(ctx context.Context, jobName, pvcName string) *reconcile.Result {
	var job batchv1.Job
	job.SetName(jobName)
	job.SetNamespace(r.managedCephClusterID)
...
`,
	...
	if err != nil {
		return reconcile.Failed("failed to create or update verify job %s: %w", jobName, err)
	}

	return nil
}

func (r *MantleBackupReconciler) hasZeroOutJobCompleted(ctx context.Context, backup *mantlev1.MantleBackup) (bool, *reconcile.Result) {
	var job batchv1.Job
	if err := r.Get(
		ctx,
...
func (r *MantleBackupReconciler) createOrUpdateImportJob(
	...
	transferPartSize, ok := backup.Status.TransferPartSize.AsInt64()
	if !ok {
		return reconcile.Failed("status.transferPartSize can't be converted to int64")
	}
	...
	if _, err := ctrl.CreateOrUpdate(ctx, r.Client, &job, func() error {
		...
		return reconcile.Failed("failed to create or update import Job: %w", err)
	}
...
func (r *MantleBackupReconciler) markSecondarySnapshotCaptured(
	...
	if err != nil {
		return reconcile.Failed("failed to find imported RBD snapshot: %w", err)
	}
...
func (r *MantleBackupReconciler) primaryCleanup(
	...
	if ok {
		...
		if err := r.Get(
			...
			return reconcile.Failed("failed to get source MantleBackup: %w", err)
		}
		delete(source.GetAnnotations(), annotDiffTo)
		if err := r.Update(ctx, &source); err != nil {
			return reconcile.Failed("failed to update source MantleBackup: %w", err)
		}
	}
	...
	if err := r.Update(ctx, target); err != nil {
		return reconcile.Failed("failed to delete annotations of diff-from and sync-mode: %w", err)
	}

	return nil
}

func (r *MantleBackupReconciler) getNumberOfPartsForResourceDeletion(backup *mantlev1.MantleBackup) (int, *reconcile.Result) {
...
func (r *MantleBackupReconciler) deleteAllExportJobs(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	return r.deleteAllJobsOfComponent(ctx, backup, MakeExportJobName)
}

func (r *MantleBackupReconciler) deleteAllUploadJobs(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	return r.deleteAllJobsOfComponent(ctx, backup, MakeUploadJobName)
}

func (r *MantleBackupReconciler) deleteAllExportDataPVCs(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	numParts, result := r.getNumberOfPartsForResourceDeletion(backup)
	if result.ShouldReturn() {
		return result.WrapIfError("failed to get the number of the parts of the exported data")
	}
...
func (r *MantleBackupReconciler) deleteVerifyJob(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	var job batchv1.Job
...
func (r *MantleBackupReconciler) deleteVerifyPV(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	var pv corev1.PersistentVolume
	pv.SetName(MakeVerifyPVName(backup))
	if err := r.Delete(ctx, &pv); err != nil && !aerrors.IsNotFound(err) {
		return reconcile.Failed("failed to delete verify PV: %w", err)
	}

	return nil
}

func (r *MantleBackupReconciler) deleteVerifyPVC(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	var pvc corev1.PersistentVolumeClaim
	pvc.SetName(MakeVerifyPVCName(backup))
	pvc.SetNamespace(r.managedCephClusterID)
	if err := r.Delete(ctx, &pvc); err != nil && !aerrors.IsNotFound(err) {
		return reconcile.Failed("failed to delete verify PVC: %w", err)
	}

	return nil
}

func (r *MantleBackupReconciler) deleteVerifyRBDImage(backup *mantlev1.MantleBackup) *reconcile.Result {
	if backup.Status.PVManifest == "" {
	...
	if pool == "" {
		return reconcile.Failed("pool name is missing in the PV manifest")
	}
	image := MakeVerifyImageName(backup)

	if err := deleteRBDImageAsynchronously(r.ceph, pool, image); err != nil {
		return reconcile.Failed("failed to delete verify RBD image: %w", err)
	}
...
func (r *MantleBackupReconciler) secondaryCleanup(
	...
	if err := r.Delete(ctx, &zeroOutPVC); err != nil && !aerrors.IsNotFound(err) {
		return reconcile.Failed("failed to delete zeroout PVC: %w", err)
	}
	...
	if err := r.Delete(ctx, &zeroOutPV); err != nil && !aerrors.IsNotFound(err) {
		return reconcile.Failed("failed to delete zeroout PV: %w", err)
	}
	...
	delete(target.GetAnnotations(), annotSyncMode)
	if err := r.Update(ctx, target); err != nil {
		return reconcile.Failed("failed to update target MantleBackup: %w", err)
	}
...
func (r *MantleBackupReconciler) deleteAllExportedData(ctx context.Context, backup *mantlev1.MantleBackup) *reconcile.Result {
	numParts, result := r.getNumberOfPartsForResourceDeletion(backup)
	if result.ShouldReturn() {
		return result.WrapIfError("failed to get the number of the parts of the exported data")
	}
...
func (r *MantleBackupReconciler) deleteMiddleSnapshots(backup *mantlev1.MantleBackup) *reconcile.Result {
	...
	transferPartSize, ok := backup.Status.TransferPartSize.AsInt64()
	if !ok {
		return reconcile.Failed("failed to get transferPartSize as int64: %s/%s", backup.GetNamespace(), backup.GetName())
	}
	...
	if slices.Contains(imageNames, image) {
		...
		if err != nil {
			return reconcile.Failed("failed to get RBD image info: %s: %s: %w", pool, image, err)
		}
		imageID = info.ID
		snaps, err = r.ceph.RBDSnapLs(pool, image)
		if err != nil {
			return reconcile.Failed("failed to list snapshots: %s: %s: %w", pool, image, err)
		}
	} else {
		...
		if err != nil {
			return reconcile.Failed("failed to list snapshots by ID: %s: %s: %w", pool, imageID, err)
		}
	}
	...
	for i := range numParts {
		...
		if err := r.ceph.RBDSnapRm(pool, imageID, snaps[snapIndex].Name); err != nil {
			return reconcile.Failed("failed to remove snapshot: %s: %s: %w", pool, image, err)
		}
	}
...
func (r *MantleBackupReconciler) updateMantleBackupCondition(
	...
	if err := r.Client.Status().Patch(ctx, willBeApplied, client.MergeFrom(target)); err != nil {
		return reconcile.Failed("failed to update MantleBackup condition by patch: %w", err)
	}
