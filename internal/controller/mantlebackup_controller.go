...
// export reconciles export Jobs and PVCs. Note that export might update
// `targetBackup`. Because of this, the values you get from r.Client.Get could
// be outdated due to its stale cache. Therefore, after we call export, we MUST
// use `targetBackup` directly and MUST NOT use r.Client.Get (nor functions like
// ctrl.CreateOrUpdate which use Get inside) to fetch its values, as they might
// not be current.
...
func (r *MantleBackupReconciler) patchStatusTransferPartSizeIfEmpty(ctx context.Context, backup *mantlev1.MantleBackup) error {
	...
	newBackup.Status.TransferPartSize = &r.backupTransferPartSize
	if err := r.Client.Status().Patch(ctx, newBackup, client.MergeFrom(backup)); err != nil {
...
func (r *MantleBackupReconciler) calcNumOfExportDataParts(snapSize int64, transferPartSizeQty *resource.Quantity) (int64, error) {
	transferPartSize, ok := transferPartSizeQty.AsInt64()
	if !ok {
		return 0, fmt.Errorf("failed to convert transferPartSize to int64: %s", transferPartSizeQty.String())
	}

	numParts := snapSize / transferPartSize
	if snapSize%transferPartSize != 0 {
		numParts++
	}
	return numParts, nil
}
...
func (r *MantleBackupReconciler) haveAllExportJobsCompleted(backup *mantlev1.MantleBackup) (bool, error) {
	if backup.Status.SnapSize == nil {
		return false, errors.New("status.snapSize is nil")
	}
	limit, err := r.calcNumOfExportDataParts(*backup.Status.SnapSize, backup.Status.TransferPartSize)
	if err != nil {
...
func (r *MantleBackupReconciler) createOrUpdateExportDataPVC(ctx context.Context, target *mantlev1.MantleBackup) error {
	...
	transferPartSize := target.Status.TransferPartSize
	pvcSizeI64, ok := transferPartSize.AsInt64()
	if !ok {
		return fmt.Errorf("failed to convert status.transferPartSize to int64: %s", transferPartSize.String())
	}
	...
	if pvcSize == nil {
		return fmt.Errorf("resource.NewQuantity failed: %d %s", pvcSizeI64, string(transferPartSize.Format))
	}
...
func (r *MantleBackupReconciler) createOrUpdateImportJob(
	...
	_, err := ctrl.CreateOrUpdate(ctx, r.Client, &job, func() error {
		...
		if partNum != 0 {
			sourceBackupName = fmt.Sprintf(
				"%s-offset-%d",
				backup.GetAnnotations()[annotRemoteUID], int64(partNum)*transferPartSize,
			)
		}
...
func (r *MantleBackupReconciler) getNumOfExportDataPartsForResourceDeletion(backup *mantlev1.MantleBackup) (int64, error) {
	if backup.Status.SnapSize == nil || backup.Status.TransferPartSize == nil {
		return 0, nil
	}
	return r.calcNumOfExportDataParts(*backup.Status.SnapSize, backup.Status.TransferPartSize)
}
...
func (r *MantleBackupReconciler) deleteAllJobsOfComponent(
	...
	numParts, err := r.getNumOfExportDataPartsForResourceDeletion(backup)
	if err != nil {
...
func (r *MantleBackupReconciler) deleteMiddleSnapshots(backup *mantlev1.MantleBackup) error {
	snapNamePrefix := fmt.Sprintf("%s-", string(backup.GetUID()))
	...
	for _, snap := range snaps {
		if !strings.HasPrefix(snap.Name, snapNamePrefix) {
			continue
		}
		if err := r.ceph.RBDSnapRm(pool, image, snap.Name); err != nil {
