...
func testFullBackup() {
	It("should correctly replicate PVC and MantleBackup resources",
		...
		testFullBackupSuccess,
	)
	It("should back up correctly if previous MB is deleted in the secondary cluster",
		testBackupLackingPreviousInSecondary)
	It("should back up correctly if previous MB is deleted in the primary cluster",
		testBackupLackingPreviousInPrimary)
}

func testFullBackupSuccess(ctx SpecContext) {
...
func testBackupLackingPreviousInSecondary(ctx SpecContext) { //nolint:dupl
	namespace := util.GetUniqueName("ns-")
	pvcName := util.GetUniqueName("pvc-")
...
func testBackupLackingPreviousInPrimary(ctx SpecContext) { //nolint:dupl
	namespace := util.GetUniqueName("ns-")
	pvcName := util.GetUniqueName("pvc-")
	backupName0 := util.GetUniqueName("mb-")
	backupName1 := util.GetUniqueName("mb-")
