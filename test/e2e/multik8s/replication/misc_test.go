...
func testMisc() {
	It("should succeed to back up if backup-transfer-part-size is changed during uploading",
		testBackupTransferPartSizeChangedDuringUploading)
	It("should succeed to back up two MantleBackups in parallel",
		testParallelBackups)
	It("should succeed in backup even if part=0 upload Job completes after part=1 upload Job does",
		testBackupSuccessWithInterleavingUploadJobs)
	It("should get metrics from the controller pod in the primary cluster",
		testMetrics)

	testBackupSuccessIfJobsTemporarilyFail()
}

func testBackupTransferPartSizeChangedDuringUploading(ctx SpecContext) {
...
func testParallelBackups(ctx SpecContext) {
	namespace := util.GetUniqueName("ns-")
...
func testBackupSuccessWithInterleavingUploadJobs(ctx SpecContext) {
	namespace := util.GetUniqueName("ns-")
	pvcName := util.GetUniqueName("pvc-")
	backupName := util.GetUniqueName("mb-")
	restoreName := util.GetUniqueName("mr-")
...
func testBackupSuccessIfJobsTemporarilyFail() {
...
func testMetrics(ctx SpecContext) {
	metrics := []string{
