...
func testIncrementalBackup() {
	It("should perform a correct incremental backup",
		testIncrementalBackupSuccess)
	It("should back up correctly if previous incremental MB is removed in the secondary cluster",
		testBackupLackingPreviousIncrementalInSecondary)
	It("should back up correctly if previous incremental MB is removed in the primary cluster",
		testBackupLackingPreviousIncrementalInPrimary)
}

func testIncrementalBackupSuccess(ctx SpecContext) {
...
func testBackupLackingPreviousIncrementalInSecondary(ctx SpecContext) {
	namespace := util.GetUniqueName("ns-")
	pvcName := util.GetUniqueName("pvc-")
	backupName0 := util.GetUniqueName("mb-")
	backupName1 := util.GetUniqueName("mb-")
...
func testBackupLackingPreviousIncrementalInPrimary(ctx SpecContext) {
	namespace := util.GetUniqueName("ns-")
	pvcName := util.GetUniqueName("pvc-")
	backupName0 := util.GetUniqueName("mb-")
	backupName1 := util.GetUniqueName("mb-")
