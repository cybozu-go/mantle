...
func testChangeToPrimary() {
	It("should cancel a full backup if the role is changed from primary to standalone",
		testCancelFullBackupIfRoleChangedFromPrimaryToStandalone)
	It("should behave correctly if role is changed from standalone to primary",
		testRoleChangeFromStandaloneToPrimary)
}

func testCancelFullBackupIfRoleChangedFromPrimaryToStandalone(ctx SpecContext) {
...
func testRoleChangeFromStandaloneToPrimary(ctx SpecContext) {
	var (
		namespace                                                                  string
	...
	*/
	...
	By("changing the role from primary to standalone", func() {
		err := ChangeClusterRole(PrimaryK8sCluster, controller.RoleStandalone)
		Expect(err).NotTo(HaveOccurred())
	})
	...
	By("creating a MantleBackup resource", func() {
	...
	By("changing the role from standalone to primary", func() {
		err := ChangeClusterRole(PrimaryK8sCluster, controller.RolePrimary)
		Expect(err).NotTo(HaveOccurred())
	})

	By("synchronizing MantleBackups correctly", func() {
