...
type multiRookCephTest struct {
	...
	mantleBackupName  string // use the same backup name for both tenants
	mantleRestoreName string // use the same restore name for both tenants
}
...
func (test *multiRookCephTest) teardownEnv() {
	for _, ns := range []string{test.tenantNamespace1, test.tenantNamespace2} {
		ns := ns
		It("delete resources in namespace: "+ns, func() {
	...
	for _, sc := range []string{test.storageClassName1, test.storageClassName2} {
		sc := sc
		It("delete storage class: "+sc, func() {
...
func (test *multiRookCephTest) testMain() {
	...
	It("create PVC, backup, and restore in tenant namespace 1", func() {
	...
	It("check the rbd images in the both clusters", func() {
		imageAndSnaps1, err := seekImageAndSnapNames(cephCluster1Namespace, test.poolName)
		Expect(err).NotTo(HaveOccurred())
		Expect(imageAndSnaps1).To(HaveLen(3))
		Expect(imageAndSnaps1).To(ConsistOf(expectImageAndSnaps1))

		imageAndSnaps2, err := seekImageAndSnapNames(cephCluster2Namespace, test.poolName)
		Expect(err).NotTo(HaveOccurred())
		Expect(imageAndSnaps2).To(HaveLen(3))
		Expect(imageAndSnaps2).To(ConsistOf(expectImageAndSnaps1))
	})

	It("create PVC, backup, and restore in tenant namespace 2", func() {
	...
	It("check the rbd images in the both clusters", func() {
		...
		imageAndSnaps2, err := seekImageAndSnapNames(cephCluster2Namespace, test.poolName)
		Expect(err).NotTo(HaveOccurred())
		Expect(imageAndSnaps2).To(HaveLen(6))
	...
	It("delete the resources in tenant namespace 1", func() {
	...
	It("check the rbd images in the both clusters", func() {
		imageAndSnaps1, err := seekImageAndSnapNames(cephCluster1Namespace, test.poolName)
		Expect(err).NotTo(HaveOccurred())
		Expect(imageAndSnaps1).To(HaveLen(1))
	...
	It("delete the resources in tenant namespace 2", func() {
	...
	It("check the rbd images in the both clusters", func() {
		...
		imageAndSnaps2, err := seekImageAndSnapNames(cephCluster2Namespace, test.poolName)
		Expect(err).NotTo(HaveOccurred())
		Expect(imageAndSnaps2).To(HaveLen(4))
	...
	It("clean up the dummy data", func() {
		...
		err := removeRBDImage(cephCluster2Namespace, test.poolName, cloneName)
		Expect(err).NotTo(HaveOccurred())
		err = removeRBDSnap(cephCluster2Namespace, test.poolName, srcImageNameDummy, test.mantleBackupName)
		Expect(err).NotTo(HaveOccurred())
		err = removeRBDImage(cephCluster2Namespace, test.poolName, srcImageNameDummy)
		Expect(err).NotTo(HaveOccurred())
	})
}
